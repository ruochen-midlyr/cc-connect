...
`)
	...
	mode, thinking, tools, thinkingMax, toolMax, _, _, _, _ := config.EffectiveDisplay(cfg, proj)
...
func TestReleaseConfig_DefaultsKeepAttachmentsAndFullDisplayEnabled(t *testing.T) {
	...
	mode, thinking, tools, _, _, _, _, _, _ := config.EffectiveDisplay(cfg, &cfg.Projects[0])
...
`)
	...
	mode, thinking, tools, _, _, _, _, _, _ := config.EffectiveDisplay(cfg, proj)
